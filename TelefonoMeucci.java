...
public class TelefonoMeucci {
    ...
    public static int countRomanPhoneNumbers(String roman) {
        ...
        for (int i = 1; i <= n; i++) {
            ...
            for (int length = 1; length <= 3; length++) {  // Longitudes posibles de números romanos (1 a 3 caracteres)
                if (i - length >= 0) {
                    ...
                    if (isRomanValid(substring, romanDigits)) {
                        dp[i] += dp[i - length];
                    }
                }
            }
    ...
    public static boolean isRomanValid(String roman, String[] romanDigits) {
        for (String validRoman : romanDigits) {
            if (roman.equals(validRoman)) {
                return true;
            }
        }
        return false;
    ...
    public static String generateRandomRomanNumber(int minLength, int maxLength) {
        ...
        for (int i = 0; i < length; i++) {
            romanNumber.append(romanSymbols[rand.nextInt(3)]); // Selecciona aleatoriamente entre I, V, X
        }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 1; i <= 5; i++) {  // Generamos 5 ejemplos aleatorios
            String randomRoman = generateRandomRomanNumber(5, 15);
            System.out.println("Ejemplo " + i + " - Número romano: " + randomRoman);
            System.out.println("Posibles números de teléfono: " + countRomanPhoneNumbers(randomRoman));
            System.out.println();
        }
    }
}